...
class LeggedRobot(BaseTask):
    ...
    def compute_reward(self):
        ...
        self.rew_buf[:] +=  self.stage_buf[:, 0]*(-torch.abs(com_height - 0.35))
        self.rew_buf[:] += self.stage_buf[:, 1]*(-torch.abs(com_height - 0.2))
        self.rew_buf[:] += self.stage_buf[:, 2]*(com_height <= 0.5)*(com_height)
        self.rew_buf[:] += self.stage_buf[:, 3]*(com_height <= 0.5)*(com_height)
        self.rew_buf[:] += self.stage_buf[:, 4]*(-torch.abs(com_height - 0.35))
        # body balance
        body_z = torch_utils.quat_rotate_inverse(self.base_quaternions, self.world_z)
        self.rew_buf[:] +=  self.stage_buf[:, 0]*(-torch.arccos(torch.clamp(body_z[:, 2], -1.0, 1.0)))
        self.rew_buf[:] += self.stage_buf[:, 1]*(-torch.arccos(torch.clamp(body_z[:, 2], -1.0, 1.0)))
        self.rew_buf[:] += self.stage_buf[:, 2]*(-torch.abs(torch.arccos(torch.clamp(body_z[:, 1], -1.0, 1.0)) - np.pi/2.0))
        self.rew_buf[:] += self.stage_buf[:, 3]*(-torch.abs(torch.arccos(torch.clamp(body_z[:, 1], -1.0, 1.0)) - np.pi/2.0))
        self.rew_buf[:] += self.stage_buf[:, 4]*(-torch.arccos(torch.clamp(body_z[:, 2], -1.0, 1.0)))
        # pitch vel
        ...
        self.rew_buf[:] +=  self.stage_buf[:, 0]*(-vel_penalty)
        self.rew_buf[:] += self.stage_buf[:, 1]*(-vel_penalty)
        self.rew_buf[:] += self.stage_buf[:, 2]*(1.0 - self.is_one_turn_buf)*(-base_ang_vel_y)
        self.rew_buf[:] += self.stage_buf[:, 3]*(1.0 - self.is_one_turn_buf)*(-base_ang_vel_y)
        self.rew_buf[:] += self.stage_buf[:, 4]*(-vel_penalty)
        # energy
        self.rew_buf[:] += -torch.square(self.dof_torques).mean(dim=-1)
        # style
        self.rew_buf[:]  += -torch.square(self.dof_positions - self.default_dof_pos).mean(dim=-1)
    ...
    def compute_observations(self):
        ...
        commands[:, 1] = masks1
        commands[:, 2] = masks2
        current_obs = torch.cat((  self.projected_gravity,
        ...
        if self.cfg.terrain.measure_heights:
            ...
            current_obs = torch.cat((self.obs_buf, heights), dim=-1)
        # add noise if needed
        if self.add_noise:
            current_obs += (2 * torch.rand_like(current_obs) - 1) * self.noise_scale_vec[0:(6 + 3 * self.num_actions)]
    ...
    def _init_buffers(self):
        ...
        self.contact_forces = gymtorch.wrap_tensor(raw_net_contact_force_tensor)
        self.rigid_body_states = gymtorch.wrap_tensor(raw_rigid_body_state_tensor)
        ...
        self.dof_positions = self.dof_state.view(self.num_envs, self.num_dof, 2)[..., 0]
        self.dof_velocities = self.dof_state.view(self.num_envs, self.num_dof, 2)[..., 1]
